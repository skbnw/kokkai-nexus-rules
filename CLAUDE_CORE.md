...
**MUST**: 外部システムの独自IDを外部キーに直接使わない。`<system>_external_ids` 対応表で受け、`external_id` には**決定論的に再現できるキー**を入れる（連番・挿入順依存の値は MUST NOT）。詳細は `COMMON_RULES.md §2-1c`。
...
| イベントの粒度に迷った（案件を1行にするか） | §5-8-c |
| 新しいソースを背骨（events）へ接続する | **§5-8-e**（収集は網羅・昇格時に選別） |
...
---
**版**: この `CLAUDE_CORE.md` は `COMMON_RULES.md` の版に追従する。参照している版: v1.8
